...
#pragma omp parallel for
    ...
    printf("Function    Best RRate MB/s Best WRate MB/s Avg time     Min time     Max time\n");
    for (j=0; j<4; j++) {
		avgtime[j] = avgtime[j]/(double)(NTIMES-1);

		printf("%s%12.1f %12.1f %11.6f  %11.6f  %11.6f\n", label[j],
	       1.0E-06 * rbytes[j]/mintime[j],
	       1.0E-06 * wbytes[j]/mintime[j],
	       avgtime[j],
	       mintime[j],
	       maxtime[j]);
    }
...
static void inline copy_chunk(STREAM_TYPE *source, STREAM_TYPE *dest, ssize_t count)
{
#ifdef PRECISION
    asm volatile(
            "mov r12, %[count]\n"
...
static void inline scale_chunk(STREAM_TYPE *source, STREAM_TYPE *dest, STREAM_TYPE scale, ssize_t count)
{
...
#else
    float vscale[8] = {scale, scale, scale, scale, scale, scale, scale, scale};

    asm volatile (
        "mov r12, %[count]\n"
        "mov r14, %[vscale]\n"
        "vmovups ymm1, YMMWORD PTR [r14 + 0]\n"
        "mov r13, %[dest]\n"
        "mov r14, %[source]\n"
...
static void inline add_chunk(STREAM_TYPE *source, STREAM_TYPE *dest, ssize_t count)
{
    ssize_t j;
...
#else
        _mm256_storeu_ps(dest + j, _mm256_add_ps(_mm256_loadu_ps(dest + j),  _mm256_loadu_ps(source + j)));
#endif
}
...
static void inline fmadd_chunk(STREAM_TYPE *source1, STREAM_TYPE *source2, STREAM_TYPE *dest, STREAM_TYPE scale, ssize_t count)
{
